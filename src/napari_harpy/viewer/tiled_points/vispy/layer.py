...
class VispyTiledPointsLayer(VispyBaseLayer[TiledPointsLayerModel]):
        ...
        napari canvas
                ...
                +-- view.scene
                        ...
                        +-- VispyTiledPointsLayer.node
                                ...
                                |       scene-node behavior from VisualNode
                                |       drawing behavior inherited from
                                |       _TiledPointsTileVisual
                                |
    ...
    ``_TiledPointsTileVisualNode`` is the scene-aware class generated by
    ``create_visual_node(_TiledPointsTileVisual)``. Every retained tile owns one
    instance, so all tile nodes share the same drawing implementation while
    owning independent vertex buffers, uniforms, visibility, and scene state.
