...
3. Implement the bounded upper threshold and ordered refine/stay/coarsen/fallback policy above. Exceeding the upper threshold disallows the ordinary stay rule; select an eligible alternative without a delay, including a finer candidate if the earlier refinement rule applies. Preserve the separately identified coarsest density fallback. Never relax the configured hard render-point limit (currently 100,000 by default) or `max_vertex_payload_bytes`; the complete retained payload must independently remain valid before reuse.
...
7. Update density-status reporting alongside the new policy. The current `"Coarsest level; above preferred screen density"` message assumes only the coarsest fallback can exceed the target. Distinguish that fallback from a level selected within hysteresis tolerance, including an above-target stay or coarsening destination. Keep actual chosen-level counts and omission metadata accurate; an allowed density overrun is informational, while a hard-limit failure retains the existing rejection diagnostics.
