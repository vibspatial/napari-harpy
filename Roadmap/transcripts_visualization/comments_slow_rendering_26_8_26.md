...
4. While Bridge is accepted and the Exact estimate remains between 50,000 and 75,000, stay at Bridge rather than immediately switching back.
5. When the Exact estimate falls below the illustrative 50,000-point refinement threshold, switch back to Exact.

Between the two thresholds, keep the previously accepted LOD when otherwise eligible. The 50,000/75,000 thresholds, including the 1.5-times-target upper tolerance, are explanatory candidates for measurement, not agreed defaults. Calibrate both thresholds and their equality rules; do not widen the band merely by pushing refinement farther below the preferred target. Keep initial and changed-selection choices on the existing policy so the comparison isolates hysteresis rather than a general increase in preferred density.

The available tolerance is limited by `hard_point_capacity`, the smaller of the configured hard point limit and the number of vertices fitting `max_vertex_payload_bytes`. When the preferred target approaches or equals that capacity, the upper tolerance is clipped. A meaningful hysteresis gap must then extend below the hard ceiling by lowering the refinement threshold. Immediate refinement arbitrarily close to that ceiling, a substantial switching gap, and strict hard-limit enforcement cannot all be guaranteed simultaneously. An over-capacity finer level must give way to an eligible coarser level, or the request must follow the existing hard-limit rejection path.

**Entry condition**

Proceed only if camera traces after Slice 12a show repeated level reversals that remain a material source of stutter, GUI frame gaps, or distracting representation switching. Use Exact/Bridge and representative Spatial boundaries, with sparse and dense distributions. The initial Slice 13 report may supplement those measurements but does not depend on 12b. If the problem is within-level viewport replacement or ordinary drawing, address that separately rather than assuming hysteresis will fix it.
...
2. Adapt the reader's metadata-only level evaluation to supply consistent evidence for the level actually chosen. The current `select_level()` stops at the first fitting level, which may differ from the hysteretic choice in either direction: hysteresis can keep a finer level above the preferred target or delay refinement from a coarser one. Return the chosen level's current estimate, positive-tile count, and omitted-value IDs together in `_LevelSelection`; simply changing its `level` field would leave incorrect metadata. Reuse the existing resident-index calculations rather than reading point payloads to decide the LOD.
3. Add a bounded upper coarsening threshold above the preferred visible-point target where hard capacity permits. An already accepted finer level may remain active above the density preference but not above that upper threshold or either hard limit. Coarsen when its estimate exceeds the upper threshold; hard-limit crossings must never wait for hysteresis. Preserve the existing coarsest-level fallback when no level meets the preferred target and the coarsest level fits both hard limits; this remains a separate exception to the density preference, not permission to exceed hard capacity. Never relax the configured hard render-point limit (currently 100,000 by default) or `max_vertex_payload_bytes`; the complete retained payload must independently remain valid before reuse.
4. Refine when the finer visible estimate crosses a calibrated lower threshold. Prefer a substantial band extending above the nominal density target over unnecessarily delaying refinement far below that target. Account explicitly for clipping at hard capacity: retain a meaningful threshold gap below the ceiling when there is no upper headroom. Choose thresholds from paired traces and document the retained-detail, drawing-cost, and any delayed-refinement trade-offs; no multiplier or fixed gap is accepted without measurement.
...
7. Update density-status reporting alongside the new policy. The current `"Coarsest level; above preferred screen density"` message assumes only the coarsest fallback can exceed the target. Distinguish that fallback from deliberately retaining a finer level within the hysteresis tolerance. Keep actual chosen-level counts and omission metadata accurate; an allowed density overrun is informational, while a hard-limit failure retains the existing rejection diagnostics.

Slice 12a makes compatible same-LOD movement cheaper; Slice 12b would make avoidable LOD reversals less frequent. Neither mechanism promises that every camera movement is a retained-batch hit. Even a small same-size pan from the original retained viewport can expose space outside its bounds and require packing and staging again. A wider hysteresis band cannot fix that replacement cost, and retaining more points can increase ordinary draw work on every frame. Do not treat fewer LOD switches alone as evidence of smoother panning.

**Focused tests if implemented**

- A finer accepted level remains selected above the preferred density target while below the calibrated upper threshold and within hard capacity. Crossing its upper threshold coarsens without a delay; crossing either hard limit cannot accept or reuse an over-capacity payload.
...
Replay identical traces with Slice 12a's unchanged-policy baseline and the proposed hysteresis. Report preferred targets, hard capacities, both transition thresholds, visible estimates, selected levels, level-switch counts, active-batch reuse and viewport replacements, packing/uploads, draw time, GUI frame gaps, and end-to-end latency. Record the extra points retained above the nominal density target and the duration or camera range over which finer detail is delayed. Include isolated gestures, repeated boundary motion, sustained pans inside and outside retained bounds, and cases where the hard ceiling clips the upper tolerance.

Accept only if reduced switching materially improves interaction or visual stability and the added draw/replacement cost of finer payloads, as well as any delayed refinement near hard capacity, is acceptable. Hard limits and correctness checks remain intact; fewer switches must not merely trade representation changes for worse frame gaps or replacement latency. Record the thresholds and evidence, or retain the original policy if hysteresis is not justified. Slice 12a, the initial Slice 13 checkpoint, and the interaction milestone can complete without this implementation. If accepted later, extend the report and affected tests; do not attribute a hysteresis gain to batch retention alone.
