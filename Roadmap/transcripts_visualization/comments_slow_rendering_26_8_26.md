...
4. **Make bucket display APIs complete-tile only and delete retired machinery.** Remove their per-request `selected_value_ids` arguments, homogeneous-mode branching, selected-range resolution, `_ResolvedSelectedValueRange`, and selected-ID synthesis once their consumers are migrated. Remove `_BucketLookupIndex`, its reader state, load/release helpers, projection and resident-byte accounting, cache-wide priming/progress/rollback code, and tests of those retired contracts. Remove helpers only after checking remaining consumers. Preserve `_BucketReaderCache` as the lazy cache of opened readers; preserve descriptor addressing, shared exact-row selection, batch output partitioning, and point-ID-free display reads. Do not add deprecated aliases or a hidden sparse-subset fallback.
5. **Update tooling and documentation together.** Audit standalone bucket/exact/acceptance benchmarks, reader fixtures, worker test doubles, and normal-viewer sparse-access guards. Remove the cache-to-canvas timing hook for `resolve_selected_tile_intervals()` and obsolete lookup metrics so deleting methods does not break instrumentation. Relabel diagnostic measurements as complete-tile reads plus filtering; do not compare them as if they still measured sparse-range IO. Update affected docstrings and runtime explanations in `CACHE_FORMAT.md` without changing its persisted schema description.
