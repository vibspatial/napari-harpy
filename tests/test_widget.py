...
def test_widget_applies_user_class_to_picked_instance(qtbot, sdata_blobs: SpatialData) -> None:
    ...
    assert table.uns[USER_CLASS_COLORS_KEY] == [UNLABELED_COLOR, CLASS_COLORS[2]]
...
def test_widget_can_clear_user_class_for_picked_instance(qtbot, sdata_blobs: SpatialData) -> None:
    ...
    assert table.uns[USER_CLASS_COLORS_KEY] == [UNLABELED_COLOR]
    assert "Current class: unlabeled." in widget.selection_status.text()
...
def test_widget_logs_warning_when_existing_user_class_colors_are_overwritten(qtbot, caplog, sdata_blobs: SpatialData) -> None:
    table = sdata_blobs["table"]
    ...
    with caplog.at_level("WARNING", logger="napari_harpy._annotation"):
        widget = HarpyWidget(viewer)

    qtbot.addWidget(widget)

    assert f"Overwriting existing `{USER_CLASS_COLORS_KEY}` palette" in caplog.text
...
def test_widget_syncs_user_class_to_backed_zarr(qtbot, backed_sdata_blobs: SpatialData) -> None:
    ...
    assert list(reread["table"].uns[USER_CLASS_COLORS_KEY]) == [UNLABELED_COLOR, CLASS_COLORS[2]]
